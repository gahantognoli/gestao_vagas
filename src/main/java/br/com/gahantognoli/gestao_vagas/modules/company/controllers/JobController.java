...
import br.com.gahantognoli.gestao_vagas.modules.company.usesCases.ListAllJobsByCompanyUseCase;
...
public class JobController {
    ...
    @GetMapping
    @PreAuthorize("hasRole('COMPANY')")
    @Tag(name = "Vagas", description = "Listagem das vagas da empresa")
    @Operation(summary = "Cadastro de vagas", description = "Essa função é responsável por listar as vagas dentro da empresa.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", content = @Content(schema = @Schema(implementation = JobEntity.class)), description = "Lista de vagas retornada com sucesso")
    })
    @SecurityRequirement(name = "jwt_auth")
    public ResponseEntity<Object> listAllJobsByCompany(HttpServletRequest request) {
        try {
            var companyId = request.getAttribute("company_id");
